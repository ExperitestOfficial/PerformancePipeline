pipeline {
  ...
  stages {
    ...
    stage('Check Performance') {
      steps {
        powershell 'echo {"filters":["app_version":1.1,"app_name":"EriBank"]"compare_to_filter":["app_name":"EriBank","app_version","1.0,0.9,0.8"] [{"name":"*","matric":"durration","acceptade_change":"5%"},{"name":"Login","matric":"cpu_avg","acceptade_change":"5%"}]}; exit 1'
      }
    }
  }
  post {
